...
import { Eye, Ear, Wind, Smile, Hand, Brain, Flame, ArrowRight, BookOpen, Layers, Clock, ShieldAlert, Ban, Activity, Heart, Anchor, Scale, RotateCcw, GripVertical } from 'lucide-react';
...
const DhammaVisualizerV2 = () => {
  ...
  // Load thứ tự và độ rộng từ localStorage khi component mount
  useEffect(() => {
    ...
    if (savedScriptureWidth) {
      setScriptureWidth(parseInt(savedScriptureWidth, 10));
    }
  }, []);

  // Save thứ tự vào localStorage
  const saveOrder = (newOrder) => {
    localStorage.setItem('dhamma-formula-order', JSON.stringify(newOrder));
  };

  // Save độ rộng vào localStorage
  ...
  const getNodeStyles = (type, baseColor) => {
    switch(type) {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex-none shadow-sm z-10">
        <div className="flex flex-col xl:flex-row xl:items-center justify-center gap-4 xl:gap-6 max-w-7xl mx-auto">
      ...
      <div ref={containerRef} className="flex flex-col lg:flex-row flex-1 overflow-hidden max-w-[1800px] mx-auto w-full relative">
        
        {/* 2. Sidebar Navigation (Formulas) */}
        <div 
          ...
          <div className="flex-1 overflow-y-auto hide-scrollbar p-2 space-y-1">
            {orderedFormulas.map((f) => (
              ...
              >
                ...
                >
                  <div className="p-1 cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600 transition-colors">
        ...
        {/* 3. Main Content Area - Split into two columns */}
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden bg-slate-50/50 min-w-0">
          
          {/* A. Flowchart Visualization Column */}
          <div className="flex-1 overflow-y-auto hide-scrollbar p-4 md:p-8 border-r border-slate-200 min-w-0">
          ...
          {/* B. Scripture Text Column */}
          <div 
            className="bg-slate-800 flex flex-col shadow-lg relative"
            style={{ width: window.innerWidth >= 1024 ? `${scriptureWidth}px` : '100%' }}
          >
